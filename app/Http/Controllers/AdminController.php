...
class AdminController extends Controller {
    ...
    public function updateCategories( Request $request )
    {
        ...
        if( $request->hasFile('thumbnail') ) {
            ...
            if( $request->file('thumbnail')->move($temp, $thumbnail) ) {
                ...
                // Delete Old Image
                Storage::disk('s3')->delete($path.$categories->$thumbnail);
    ...
    public function themeStore(Request $request)
    {
        ...
        if( $request->hasFile('logo') )	{
            ...
            if($request->file('logo')->move($temp, $file) ) {
                Storage::disk('s3')->put($path.$file, $temp.$file, 'public');
    ...
    {
        $sql = AdminSettings::first();
        $rules = array('paypal_account' => 'email');
        $this->validate($request, $rules);
    ...
    public function withdrawalsView($id)
    {
        ...
        return view('admin.withdrawal-view', [
            'data' => $data,
            'settings' => $this->settings
        ]);
    }//<--- End Method
