    ...
    public function update_password(Request $request)
    {
        ...
        if (!\Hash::check($input['old_password'], Auth::user()->password) ) {
            return redirect('account/password')
                ->with( array( 'incorrect_pass' => trans('misc.password_incorrect') ) );
        }
        ...
        if( $request->hasFile('photo') )	{
            ...
            if( $request->file('photo')->move($temp, $avatar) ) {
                ...
                if ( \File::exists($temp.$avatar) ) {
                    /* Avatar */
                    Storage::disk('s3')->put($path.$avatar, file_get_contents($temp.$avatar), 'public');
                ...
                if (Storage::disk('s3')->exists($imgOld) && $imgOld != $path.'default.jpg') {
                    Storage::disk('s3')->delete($imgOld);
                    \File::delete($temp.$avatar);
                    \File::delete($imgOld);
                }//<--- IF FILE EXISTS #1
        ...
        if( $request->hasFile('photo') )	{
            ...
            if( $request->file('photo')->move($temp, $cover) ) {
                ...
                if ( \File::exists($temp.$cover) ) {
                    /* Avatar */
                    Storage::disk('s3')->put($path.$cover, $temp.$cover, 'public');
                ...
                if (Storage::disk('s3')->exists($imgOld) && $imgOld != $path.'cover.jpg' ) {
                    \File::delete($temp.$cover);
                    Storage::disk('s3')->delete($imgOld);
                    // \File::delete($imgOld);
                }//<--- IF FILE EXISTS #1
