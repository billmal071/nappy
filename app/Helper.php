...
class Helper
{
    ...
    public static function getUrlFromS3(string $path, string $filename = ''): string
    {
        return Storage::disk('s3')->url(config($path).$filename);
    }

    public static function imgixUrl(string $path, string $filename = '', string $options = ''): string
    {
        ...
        return 'https://nappy.imgix.net/'.config($path).$filename.'?'.$query.$options;
    }
