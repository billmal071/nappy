				...
				<div class="media li-group noty-media">
					<div class="pull-left">
						<a href="{{ url('/') }}/{{ $key->username }}">
							<img
								loading="lazy"
								@if($key->avatar == 'default.jpg')
									src={{ url('public/avatar', 'default.jpg') }}
								@else
									src="{{ App\Helper::imgixUrl('path.avatar', $key->avatar) }}"
								@endif
								width="40"
								height="40"
					...
					<div class="media-body">
						...
						<h5 class="media-heading"><a href="{{ url('/') }}/{{ $key->username }}"> <strong>@if( $key->name != '') {{ e( $key->name ) }} @else {{ e( $key->username )  }} @endif</strong> </a></h5>
