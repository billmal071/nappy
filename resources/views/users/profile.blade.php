...
@section('content')
    <div class="jumbotron profileUser index-header jumbotron_set jumbotron-cover-user hide-cover" style="{{$cover}}">
        <div class="container wrap-jumbotron position-relative">
            @if( Auth::check() && Auth::user()->id == $user->id )
                <!-- *********** COVER ************* -->
                <form
                    class="pull-left myicon-right position-relative"
                    style="z-index: 100;"
                    action="{{url('upload/cover')}}"
                    method="POST"
                    id="formCover"
                    ...
                    <button
                        ...
                        <i class="icon-camera myicon-right"></i>
                        {{ trans('misc.change_cover') }}
                    </button>
        ...
        <div class="row"></div>
            ...
            <div class="col-md-12">
                <div class="center-block text-center profile-user-over">
                    ...
                    <h1 class="margins title-item none-overflow font-default">
                        @if( $user->name != '' )
                            <span>
                                {{ e( $user->name ) }}
                            </span>
                    ...
                    @if( Auth::check() && $user->id != Auth::user()->id )
                        <button
                            ...
                            <i class="glyphicon glyphicon{{ $icoFollow }} myicon-right"></i>
                            {{ $textFollow }}
                        </button>
                    @endif

                    {{-- @if( Auth::check() && $user->id != Auth::user()->id && $user->paypal_account != '' || Auth::guest()  && $user->paypal_account != '' ) --}}
                    @if( Auth::check() && $user->id != Auth::user()->id || Auth::guest())
                        <button
                            type="button"
                            class="btn btn-sm btn-outline-dark"
                            style="border-color: black; margin: 1rem auto;"
                            id="btnFormPP_prof">
                            {{trans('misc.donate')}}
                        </button>
                    @endif
                    ...
                    @if( $user->countries_id != '' )
                        <small class="center-block subtitle-user" style="margin: 2rem auto;">
                            <i class="fa fa-map-marker myicon-right"></i>
                    ...
                    {{-- @elseif($user->bio == '' && Auth::check() && $user->id == Auth::user()->id)
                        <h6 class="text-center bio-user none-overflow">{{ e($user->bio) }}</h6>
                        <h5 class="text-center bio-user none-overflow">Todo: edit bio</h5> --}}
                    @endif
    ...
    @if( Auth::check() &&
        ...
        $user->paypal_account != '' || Auth::guest() &&
        $user->paypal_account != '' )
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body listWrap">
                        ...
                        <form method="POST" action="{{ url('report/user') }}" enctype="multipart/form-data" id="formReport">
                            ...
                            <button type="submit" class="btn btn-sm btn-danger reportUser">
                                {{ trans('misc.report') }}
                            </button>
                        </form>
                    </div><!-- Modal body -->
